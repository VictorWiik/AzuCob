...
export default function Settings() {
  ...
  const handleSync = async (type: 'clients' | 'receivables' | 'efi' | 'full') => {
    if ((type === 'receivables' || type === 'full') && filterOption === 'custom' && !customDate) {
    ...
    try {
      ...
      switch (type) {
        case 'clients':
          result = await api.syncClients()
          break
        case 'receivables':
          result = await api.syncReceivables(getFilterParams())
          break
        case 'efi':
          result = await api.syncEfi()
          break
        case 'full':
          result = await api.fullSync(getFilterParams())
          break
      }
      
      toast.success(result?.message || 'Sincronização realizada!')
      fetchStatus()
    } catch (error) {
  ...
  const handleProcessCharges = async () => {
    ...
    try {
      const data = await api.processCharges()
      toast.success(`Processamento concluído! ${data?.result?.sent || 0} emails enviados.`)
    } catch (error) {
            ...
            <div className="space-y-6">
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                <h4 className="font-semibold text-purple-800 mb-3 flex items-center gap-2">
                  ...
                  Filtro de Período (Contas a Receber)
                </h4>
                <p className="text-purple-700 text-sm mb-4">
                  Selecione o período para sincronização das contas em atraso
                </p>
                ...
                {filterOption === 'custom' && (
                  <div className="mt-4">
                    ...
                    <input
                      ...
                      className="w-full md:w-64 px-4 py-2 border border-purple-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                    />
                  </div>
                )}
              </div>
              ...
              <div className="grid gap-4">
                ...
                <div className="flex items-center justify-between p-4 border rounded-lg bg-purple-50/50">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      Período: {filterOption === 'custom' ? (customDate || 'Selecione uma data') : `Últimos ${filterOption} dias`}
                    </p>
                    <p className="text-sm text-gray-500">
