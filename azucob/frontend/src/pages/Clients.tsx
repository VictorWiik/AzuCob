      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <>
            ...
            {/* Pagination */}
            {clients && clients.meta.totalPages > 1 && (
              <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100">
                <p className="text-sm text-gray-500">
                  Mostrando {(clients.meta.page - 1) * clients.meta.limit + 1} a{' '}
                  {Math.min(clients.meta.page * clients.meta.limit, clients.meta.total)} de{' '}
                  {clients.meta.total} clientes
                </p>
                <div className="flex gap-2">
                  <button
                  ...
                  <button
                    onClick={() => navigate(`/clients?page=${page + 1}${filter ? `&filter=${filter}` : ''}`)}
                    disabled={page === clients.meta.totalPages}
                    className="p-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
