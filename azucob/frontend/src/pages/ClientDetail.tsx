...
export default function ClientDetail() {
  ...
  const loadClient = async () => {
    ...
    try {
      ...
      // Os receivables vêm junto com o cliente
  ...
  const handleSettleReceivable = async (receivableId: string, value: number) => {
    const paymentDate = prompt('Data do pagamento (AAAA-MM-DD):', new Date().toISOString().split('T')[0])
    if (!paymentDate) return

    const paidValue = prompt('Valor pago:', value.toString())
    if (!paidValue) return
            ...
            <div className="space-y-2">
              {client.additionalEmails && client.additionalEmails.length > 0 ? (
                client.additionalEmails.map((email) => (
                  ...
                  >
                    <div>
                      <p className="text-sm font-medium text-gray-800">{email.email}</p>
                      {email.name && (
                        <p className="text-xs text-gray-400">{email.name}</p>
                      )}
                    </div>
                    <button
                  ...
                  <div key={receivable.id} className="p-4">
                    <div className="flex items-start justify-between">
                      ...
                      <p className="text-lg font-bold text-red-600">
                        {formatCurrency(Number(receivable.value))}
                      </p>
                    </div>
                    <div className="flex gap-2 mt-3">
                      ...
                      <button
                        onClick={() => handleSettleReceivable(receivable.id, Number(receivable.value))}
