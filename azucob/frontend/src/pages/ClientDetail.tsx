...
export default function ClientDetail() {
  ...
  const loadClient = async () => {
    if (!id) return
    try {
      const [clientData, receivablesData] = await Promise.all([
        api.getClient(id),
        api.getReceivables({ clientId: id }),
      ])
  ...
  const handleSendCharge = async (receivableId: string) => {
    setSendingCharge(receivableId)
    try {
      const result = await api.sendCharge(receivableId)
  ...
  const handleSettleReceivable = async (receivableId: string) => {
    ...
    try {
      await api.settleReceivable(receivableId, paymentDate)
